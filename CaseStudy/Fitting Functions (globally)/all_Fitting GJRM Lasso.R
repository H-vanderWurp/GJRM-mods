...
exe.cop.lasso.all <- function(Cop, date.ind){
  library(EUfootball)
  require(GJRM)
  ...
  #repeat{
    # indep equations
    print(dim(test))
    eq1 <- Goals90Home ~ eloHome + MWHomeT + pHome + FormGoals3Home +
      AufsteigerHome + TitelHome + PokalHome
    eq2 <- Goals90Guest ~ eloGuest + MWGuestT + pGuest + FormGoals3Guest +
      AufsteigerGuest + TitelGuest + PokalGuest
    eq3 <- ~ 1
