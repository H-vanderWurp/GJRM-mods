...
exe.pois.lasso.both.all <- function(Liga){
  library(EUfootball)
  ...
  repeat{
    ...
    if(!is.matrix(testhere1)){
      testhere1 <- testhere1 - rep(nu1, each = 1)
      testhere2 <- testhere2 - rep(nu2, each = 1)
      testhere1 <- testhere1 / rep(sigma1, each = 1)
      testhere2 <- testhere2 / rep(sigma2, each = 1)
    }
    else{
    ...
    Res$matchday[index1:index2] <- testhere.full$matchday
    Res$League[index1:index2] <- testhere.full$League
  ...
  save(file = paste0("Results (globally)/pois_lasso_both.rData"), Res)
}
