## Fitting Cop, equal+lasso penalty both

## can easily be modified to only predict a single matchday. 
## repeat()-loop is removed and with a new argument matchday the dataset
## is split into train and test. Done (and applied on computational cluster).
## Results are from there.

## year. Starting in 2015.

exe.cop.lasso.equal.both <- function(Liga, Cop, s.year = 2015){
  require(EUfootball)
  Dat <- Matches
  ...
  ## only for CV: factor into numeric
  Dat$PromotedHome <- as.numeric(Dat$PromotedHome)
  Dat$PromotedGuest <- as.numeric(Dat$PromotedGuest)
  Dat$TitleholderHome <- as.numeric(Dat$TitleholderHome)
  Dat$TitleholderGuest <- as.numeric(Dat$TitleholderGuest)
  Dat$CupTitleholderHome <- as.numeric(Dat$CupTitleholderHome)
  Dat$CupTitleholderGuest <- as.numeric(Dat$CupTitleholderGuest)
  ## only for CV: complete cases only
  Dat <- Dat[!is.na(Dat$MVHome.T) & !is.na(Dat$MVGuest.T),]
  ...
  repeat{
    ...
    a <- try(fitcop <- gjrm.lasso(data = list(train, formula = list(eq1, eq2, eq3)), Cop = Cop,
                   grid.l = 25, threshold = 0.01, carry.start.values = TRUE,
                   start.nu = 20, CV = FALSE, plot = FALSE, 
  ...
  Res <- list(ResAIC = ResAIC, ResBIC = ResBIC, 
              ...
              errors = errorcount)
  save(file = paste0("Results/cop_lasso+equal_both/Res_", Liga, s.year, "_", Cop, ".rData"), Res)
}
