                  ...
                  xi = 1e9, conv.crit = 0.01, iterative = TRUE) 
{
  ...
  if ((!(margins[1] %in% bl) || surv == TRUE) && ordinal == 
    ...
    if(LASSO){
      ...
      ##hv: Groups of dummy variables have to be treated seperately. The values from
    ...
    ##hv: transformation back
    if(LASSO){
      SemiParFit$fit$argument <- SemiParFit$fit$argument / c(sds, replicate(VC$gp3, 1))
                            ...
                            xi = 1e9, conv.crit = 0.01, iterative = FALSE) 
{
  ...
  if(LASSO & LASSO.nu > 0) {
    ...
    d <- nrow(K.blank)
    K.blank[d,d] <- 0 ##hv: no penalty on third margin (copula parameter)
  ...
  while(difcrit > conv.crit){
    ...
    ##hv: new
    if(iterative){
      difcrit <- sum(abs(fit$argument - start.v)) / sum(abs(start.v))
